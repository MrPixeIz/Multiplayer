...
public class SpawnManager : NetworkBehaviour {
    ...
    [Command]
    private void CmdSpawnManager() {
    GameObject scoreDisplayer =(GameObject)Instantiate(scoreDisplayerPrefab);
        NetworkServer.Spawn(scoreDisplayer);
        GameObject respawnManager = (GameObject)Instantiate(playerRespawnManagerPrefab);
        NetworkServer.Spawn(respawnManager);
    }

    [Command]
    private void CmdSpawnOscillator()
    {
